...
const CultureScreen = ({ navigation, route }) => {
  ...
  async function playSound() {
    ...
    const { sound } = await Audio.Sound.createAsync(culture.audio);
    setSound(sound);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      ...
      <ImageBackground style={{ flex: 0.5 }} source={culture.image}>
          ...
          <View style={{ marginLeft: 30, flexDirection: "row" }}>
            <TouchableOpacity style={{ padding: 8 }}>
              <AntDesign
                ...
                size={25}
                color={COLORS.red}
                onPress={() => Linking.openURL(culture.url)}
              />
            </TouchableOpacity>
